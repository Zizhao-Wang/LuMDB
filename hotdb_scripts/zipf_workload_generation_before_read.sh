...
for i in {10..10}; do
    base_num=$(($billion * $i))
    dir1="${i}B_LuMDB_POint_Query_perfromance"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
        for value_size in 128; do
            ...
            for zipf_a in 1.1; do  # 1.2 1.3 1.4 1.5
                ...
                log_file="LuMDB_POint_Query_${num_format}_val_${value_size}_mem${Mem}MiB_zipf${zipf_a}.log"
                ...
                db_directory="/mnt/hotdb_test/hotdb10B/write_mem${Mem}MiB_zipf${zipf_a}"
