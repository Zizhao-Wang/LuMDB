...
OUTPUT_DIR=./AcerN3500_fs_fio_test_write_ranseq
...
do
    ...
    do
        ...
        do
            ...
            SECOND_SMALLEST_FIO_PID=$(pgrep -af "fio --name=rand_write_test" | awk '{print $1}' | sort -n | sed -n '2p')
            # 检查是否有进程ID被捕获
            ...
            sleep 1
            # 使用 pgrep 捕获所有相关的 fio 进程ID，排序后选取第二个最小的 PID
            SECOND_SMALLEST_FIO_PID=$(pgrep -af "fio --name=seq_write_test" | awk '{print $1}' | sort -n | sed -n '2p')
            # 检查是否有进程ID被捕获
            if [ -z "$SECOND_SMALLEST_FIO_PID" ]; then
                echo "No suitable FIO process found. Exiting."
                exit 1
