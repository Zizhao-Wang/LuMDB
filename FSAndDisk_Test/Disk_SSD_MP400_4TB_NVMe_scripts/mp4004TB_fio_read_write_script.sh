...
OUTPUT_DIR=./mp4004TB_fio_test_readwrite_ranseq
# 设定文件路径和大小
...
do
    ...
    # 随机写测试
    iostat -d $IOSTAT_INTERVAL -x $FILENAME > $OUTPUT_DIR/rand_write_test_${NUMJOBS}_jobs_IOstats.out &
    PID_IOSTAT=$!
    ...
    # 顺序写测试
    iostat -d $IOSTAT_INTERVAL -x $FILENAME > $OUTPUT_DIR/seq_write_test_${NUMJOBS}_jobs_IOstats.out &
    PID_IOSTAT=$!
