OUTPUT_DIR=./mp4008TB_fio_test_readwrite_ranseq
# 设定文件路径和大小
# 使用从父脚本导出的FILENAME变量
...
do
    ...
    # 随机写测试
    iostat -d 1 -x $FILENAME > $OUTPUT_DIR/rand_write_test_${NUMJOBS}_jobs_IOstats.out &
    PID_IOSTAT=$!
    ...
    # 顺序写测试
    iostat -d 1 -x $FILENAME > $OUTPUT_DIR/seq_write_test_${NUMJOBS}_jobs_IOstats.out &
    PID_IOSTAT=$!
