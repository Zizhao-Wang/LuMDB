...
OUTPUT_DIR=./mp4004TB_fs_fio_test_read_ranseq
    ...
    do
        ...
        do
            ...
            if [ -f "$FIO_OUTPUT_FILE" ]; then
                ...
                FIO_COMMAND_PATTERN="fio --name=rand_read_test --rw=randread --bs=$BLOCK_SIZE --size=$SIZE_PER_JOB --numjobs=$NUMJOBS --ioengine=libaio --direct=1 --filename=$FILENAME"                                             
            ...
            if [ -f "$FIO_OUTPUT_FILE" ]; then
                ...
                sleep 1
                # 使用 pgrep 捕获所有相关的 fio 进程ID，排序后选取第二个最小的 PID
                FIO_COMMAND_PATTERN="fio --name=seq_read_test --rw=read --bs=$BLOCK_SIZE --size=$SIZE_PER_JOB --numjobs=$NUMJOBS --ioengine=libaio --direct=1 --filename=$FILENAME"                                                            
                # SECOND_SMALLEST_FIO_PID=$(pgrep -af "$FIO_COMMAND_PATTERN" | awk '{print $1}' | sort -n | sed -n '2p')
                # # 检查是否有进程ID被捕获
                # if [ -z "$SECOND_SMALLEST_FIO_PID" ]; then
                #     echo "No suitable FIO process found. Exiting."
