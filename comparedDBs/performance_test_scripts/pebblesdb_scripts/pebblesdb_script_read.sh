    ...
    if [ ! -d "$dir1" ]; then
        ...
        for value_size in 128; do
            ...
            for zipf_a in 1.2; do  #1.1  1.2 1.3 1.4 1.5
                ...
                log_file="Pebbles10B_${num_format}_val_${value_size}_mem${Mem}MB_zipf${zipf_a}_EXIST1.log"
                reads_data_file="random_read_workload${zipf_a}.csv"
                ...
                cgexec -g memory:group16 ../../../pebblesdb/release/db_bench \
                --db=$db_dir \
                --num=$num_entries \
