...
for i in {10..10}; do
    base_num=$(($billion * $i))
    dir1="${i}B_leveldb_Range_Query_perfromance"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
            ...
            for zipf_a in 1.2; do  #  1.2 
                    ...
                    for no_hot in "${No_hot_percentages[@]}"; do
                        ...
                        for buffer_size in 1048576; do
                            ...
                            log_file="LevelDB_Range_Query${Query_lengrh}_${num_format}_val_${value_size}_mem${buffer_size_mb}MB_zipf${zipf_a}.log"
                            ...
                            --open_files=40000 \
                            --range_query_length=$Query_lengrh \
                            --compression=0 \
