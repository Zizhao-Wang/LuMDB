...
for i in {10..10}; do
    base_num=$(($billion * $i))
    dir1="${i}B_leveldb_zipf_hot_removal_SATASSD"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
            ...
            for zipf_a in 1.5 1.4 1.3 1.2 1.1 ; do  #  1.2 
                    ...
                    for no_hot in "${No_hot_percentages[@]}"; do
                        ...
                        for buffer_size in 67108864 33554432 16777216 8388608 1048576; do
                            ...
                            memory_log_file="/home/jeff-wang/WorkloadAnalysis/comparedDBs/performance_test_scripts/leveldb_scripts/10B_leveldb_zipf_hot_removal_SATASSD/leveldb_memory_usage_${num_format}_key16_val${value_size}_${buffer_size_mb}MB.log"
