        ...
        for value_size in 128; do
            ...
            for zipf_a in 1.1  ; do  #  1.2 
                for buffer_size in 1048576; do
                    ...
                    memory_log_file="/home/jeff-wang/WorkloadAnalysis/comparedDBs/performance_test_scripts/leveldb_augmentation/10B_leveldb_augmentation/leveldbf${F}_memory_usage_${num_format}_key16_val${value_size}_factor${F}_level1base${level1base}MiB.log"  
                    ...
                    # 创建相应的目录
                    db_dir="/mnt/hotdb_test/level10B/leveldb_augmentation/f${F}_${zipf_a}_level1base_${level1base}"
                    if [ ! -d "$db_dir" ]; then
                        mkdir -p "$db_dir"
                    fi
