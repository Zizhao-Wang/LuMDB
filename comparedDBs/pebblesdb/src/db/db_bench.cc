...
class KeyBuffer {
 public:
  ...
  void Set(int k) {
    std::snprintf(buffer_, sizeof(buffer_), "%016d", k);
  }

  Slice slice() const { return Slice(buffer_, 16); }
...
class variable_Buffer{
 public:
  ...
  void Set(uint64_t k,int key_size) {
    ...
    assert(key_size <= sizeof(buffer_) );
    char format[20];
    std::snprintf(format, sizeof(format), "%%0%dllu", key_size);
    std::snprintf(buffer_ , sizeof(buffer_) , format, (unsigned long long)k);
    this->key_sizes = key_size;
  }

  void PrintBuffer() const {
    std::cout.write(buffer_,  key_sizes);
    std::cout << std::endl;
  }

  Slice slice() const { return Slice(buffer_,  this->key_sizes); }
...
//        entries_per_batch_ = 1000;
 ...
 private:
  ...
  void WriteEtc(ThreadState* thread) {
    DoWrite2(thread, false);
  }

  void Writezipf(ThreadState* thread) {
    DoWrite_zipf(thread, false);
  }
  ...
  void DoWrite2(ThreadState* thread, bool seq) {
    ...
    fprintf(stdout,"Entering ETC workload testing:\n");
    fflush(stdout);
