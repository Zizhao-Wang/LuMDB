...
DBImpl::DBImpl(const Options& raw_options, const std::string& dbname)
    : env_(raw_options.env),
      ...
      bg_error_(),
      num_bg_compaction_threads_(options_.bg_threads) {
  mutex_.Lock();
  mem_->Ref();
...
void DBImpl::DeleteObsoleteFiles() {
  ...
  {
    filenames.clear();
    env_->GetChildren(options_.log_path, &filenames); // Ignoring errors on purpose
    for (size_t i = 0; i < filenames.size(); i++) {
      if (ParseFileName(filenames[i], &number, &type)) {
        bool keep = true;
        switch (type) {
        ...
        if (!keep) {
          Log(options_.info_log, "Delete type=%d #%lld\n",
              int(type),
              static_cast<unsigned long long>(number));
          env_->DeleteFile(options_.log_path + "/"+ filenames[i]);
        }
      }
    }
  }
...
Status DBImpl::RecoverLogFile(uint64_t log_number,
  ...
  // Open the log file
  std::string fname = LogFileName(options_.log_path, log_number);
  SequentialFile* file;
...
void DBImpl::CompactLevelThread() {
  ...
  while (!shutting_down_.Acquire_Load()) {
    ...
    assert(manual_compaction_ == NULL );
...
Status DBImpl::Write(const WriteOptions& options, WriteBatch* updates) {
  ...
  if (s.ok() && updates != NULL) { // NULL batch is for compactions
    ...
    if (options_.log) s = w.log_->AddRecord(WriteBatchInternal::Contents(updates_with_guards));
...

  {
    ...
    while (true) {
      if (!bg_error_.ok()) {
        ...
        Log(options_.info_log, "Imm Full. Waiting...");
        auto start = env_->NowMicros();
        bg_memtable_cv_.Signal();
        bg_fg_cv_.Wait();
        auto duration = env_->NowMicros() - start;
        Log(options_.info_log, "Imm Full. Waiting finish: %d us", duration);
      } else {
        // Attempt to switch to a new memtable and trigger compaction of old
        assert(versions_->PrevLogNumber() == 0);
        uint64_t new_log_number = versions_->NewFileNumber();
        ConcurrentWritableFile* lfile = NULL;
        s = env_->NewConcurrentWritableFile(LogFileName(options_.log_path, new_log_number), &lfile);
        if (!s.ok()) {
...
Status DB::Open(const Options& options, const std::string& dbname,
  ...
  *dbptr = NULL;
  ...
  printf("Recover Time : %llu\n", start - end);

  if (s.ok()) {
    uint64_t new_log_number = impl->versions_->NewFileNumber();
    ConcurrentWritableFile* lfile;
    s = options.env->NewConcurrentWritableFile(LogFileName(options.log_path, new_log_number),
                                               &lfile);
    if (s.ok()) {
