...
def process_csv(input_file, n, m, k, output_ops_file):
    ...
    # 为读取和更新操作添加操作类型
    read_ops = pd.DataFrame({'Operation': 'READ', 'Key': update_keys})
    update_ops = pd.DataFrame({'Operation': 'UPDATE', 'Key': read_keys})
...
m = 50000   # 从去重后的数据中随机选取 m 个读取操作
k = 950000  # 从第 n+1 到 n+k 行数据提取 k 个更新操作

output_ops_file = '/home/jeff-wang/workloads/ycsb_b_workload.csv'  # 最终操作数据保存路径
